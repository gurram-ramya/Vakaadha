...
def remove_from_wishlist(product_id):
    try:
        ...
        logging.exception("wishlist delete failed")
        return jsonify({"error": "ServerError", "details": str(e)}), 500
...
def get_wishlist():
    try:
        user_id, guest_id = get_context()
        result = wishlist_service.get_wishlist(user_id, guest_id)
        return jsonify(result), 200
    except Exception as e:
...
def get_wishlist_count():
    try:
        ...
        logging.exception("wishlist count failed")
        return jsonify({"error": "ServerError", "details": str(e)}), 500
...
def clear_wishlist():
    try:
        ...
        logging.exception("wishlist clear failed")
        return jsonify({"error": "ServerError", "details": str(e)}), 500
...
def move_to_cart():
    body = request.get_json(force=True)
    if not body or "product_id" not in body or "variant_id" not in body:
        return jsonify({"error": "Missing product_id or variant_id"}), 400
    ...
    try:
        ...
        logging.exception("wishlist move-to-cart failed")
        return jsonify({"error": "ServerError", "details": str(e)}), 500
...
def archive_wishlist(wishlist_id):
    try:
        ...
        logging.exception("wishlist archive failed")
        return jsonify({"error": "ServerError", "details": str(e)}), 500
