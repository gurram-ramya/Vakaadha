...
def create_order_from_cart(user_id, cart_id, address_id, payment_method):
    with transaction() as cur:
        cart_items = cart_repo.get_cart_items(cart_id)
        totals = cart_repo.compute_cart_totals(cart_id)
        ...
        order_no = f"ORD-{cart_id}-{user_id}"
        ...
        cart_repo.mark_cart_converted(cart_id)
...
def list_user_orders(user_id):
    rows = repo.get_orders_by_user(user_id)
    return rows

def get_order_details(order_id):
    data = repo.get_order_details(order_id)
    if not data:
        return None

    order = dict(data["order"])
    order["items"] = [dict(i) for i in data["items"]]
    return order
...
def update_payment_status(order_id, payment_status):
    with transaction():
        repo.update_payment_status(order_id, payment_status)

def get_order_confirmation_details(order_id):
    data = repo.get_order_with_items_and_address(order_id)
    if not data:
        return None

    order = dict(data["order"])
    items = [dict(i) for i in data["items"]]
    address = dict(data["address"]) if data["address"] else {}
